            ...
            for base in cls.__bases__:
                if base.__name__ not in ('object', 'ArchiveSection', 'Section'):
                    ...
                    if base.__module__ != package_name:
                        ...
                        f.write(f"  class {base.__name__} {{\n    <<from {base.__module__}>>\n  }}\n")
                        
                        # Add click event
                        if base.__module__.startswith('nomad.'):
                            f.write(f"  click {base.__name__} href \"https://nomad-lab.eu/docs/reference/datamodel/datamodel.html\" \"View {base.__name__} Docs\"\n")
                        else:
                            # Link to internal generated docs
                            base_pkg_short = base.__module__.split('.')[-2] if '.' in base.__module__ else base.__module__
                            f.write(f"  click {base.__name__} href \"schema_{base_pkg_short}.html\" \"View {base.__name__} Schema\"\n")
            ...
            if hasattr(cls, 'm_def'):
                for sub in cls.m_def.sub_sections:
                    ...
                    if sub.sub_section.section_cls.__module__ != package_name:
                        mod_short = sub.sub_section.section_cls.__module__.split('.')[-1]
                        f.write(f"  class {sub_sec_name} {{\n    <<from {mod_short}>>\n  }}\n")
                        
                        # Add click event for subsection
                        sub_mod = sub.sub_section.section_cls.__module__
                        if sub_mod.startswith('nomad.'):
                            f.write(f"  click {sub_sec_name} href \"https://nomad-lab.eu/docs/reference/datamodel/datamodel.html\" \"View {sub_sec_name} Docs\"\n")
                        else:
                            sub_pkg_short = sub_mod.split('.')[-2] if '.' in sub_mod else sub_mod
                            f.write(f"  click {sub_sec_name} href \"schema_{sub_pkg_short}.md\" \"View {sub_sec_name} Schema\"\n")
